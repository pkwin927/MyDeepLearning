...
#aaa = mnist.train.next_batch(5)
#
#zzz = tf.reshape(aaa,[-1,28,28,1])
#
#sess.run(zzz)
...
#b_x,b_y = mnist.train.next_batch(1)
#
#b_x = mnist.train.next_batch(1)
#
#
#sess.run(image, feed_dict = {tf_x:b_x, tf_y:b_y})
...
#for step in range(600):
#    b_x, b_y = mnist.train.next_batch(BATCH_SIZE)
#     sess.run(image, {tf_x: b_x, tf_y: b_y})
#    if step % 50 == 0:
#        accuracy_, flat_representation = sess.run([accuracy, flat], {tf_x: test_x, tf_y: test_y})
#        print('Step:', step, '| train loss: %.4f' % loss_, '| test accuracy: %.2f' % accuracy_)
#
