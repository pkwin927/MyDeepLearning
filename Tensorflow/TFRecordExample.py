...
# -*- coding: utf-8 -*-
"""
Created on Wed Oct  3 04:52:11 2018
...
with tf.device('/cpu:0'):
    ...
    def model(x_crop, y_, reuse):
        ...
        with tf.variable_scope("model", reuse=reuse):
            ...
            net = tl.layers.Conv2d(net, 64, (5, 5), (1, 1), act=tf.nn.relu, padding='SAME', W_init=W_init, name='cnn2')
            ...
            net = tl.layers.DenseLayer(net, 384, act=tf.nn.relu, W_init=W_init2, b_init=b_init2, name='d1relu')
            net = tl.layers.DenseLayer(net, 192, act=tf.nn.relu, W_init=W_init2, b_init=b_init2, name='d2relu')
            net = tl.layers.DenseLayer(net, n_units=10, act=None, W_init=W_init2, name='output')
            y = net.outputs
    ...
    def model_batch_norm(x_crop, y_, reuse, is_train):
        ...
        with tf.variable_scope("model", reuse=reuse):
            ...
            net = tl.layers.BatchNormLayer(net, decay=0.99, is_train=is_train, act=tf.nn.relu, name='batch1')
            net = tl.layers.MaxPool2d(net, (3, 3), (2, 2), padding='SAME', name='pool1')

            net = tl.layers.Conv2d(net, 64, (5, 5), (1, 1), padding='SAME', W_init=W_init, b_init=None, name='cnn2')
            net = tl.layers.BatchNormLayer(net, decay=0.99, is_train=is_train, act=tf.nn.relu, name='batch2')
            net = tl.layers.MaxPool2d(net, (3, 3), (2, 2), padding='SAME', name='pool2')
